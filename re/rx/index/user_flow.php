...
💬 مبلغ باید حداقل $minbalance تومان و حداکثر $maxbalance تومان باشد", null, 'HTML');
    ...
    if ($datain == "cart_to_offline") {
        ...
        if ($PaySetting == "onautoconfirm") {
            ...
            // دو حالت خراب باعث می‌شد شماره کارت اصلاً نمایش داده نشود:
            //   ۱) قالب text_cart خالی مانده باشد.
            //   ۲) قالب به‌اشتباه با یک متنِ نامربوط (مثلاً برچسبِ دکمه‌ی ادمینِ
            //      «⚙️ وضعیت قابلیت ها») پُر شده باشد؛ چون آن‌وقت رشته خالی نیست،
            //      چکِ خالی‌بودن فعال نمی‌شد و همان زباله به‌جای کارت ارسال می‌شد.
            // معیارِ درست: اگر متنِ رندرشده شماره‌ی کارت را در خودش نداشته باشد،
            // یعنی قالب placeholderِ {card_number} را ندارد یا خراب است — پس از
            // fallbackِ امن استفاده می‌کنیم که همیشه کارت و مبلغ را نشان می‌دهد.
            if (
                trim(strip_tags((string) $textcart)) === ''
                || strpos((string) $textcart, (string) $card_number) === false
            ) {
